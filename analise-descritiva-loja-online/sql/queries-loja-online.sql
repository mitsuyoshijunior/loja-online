...
SELECT 
    ...
    vpm.Total_vendido,
    dpm.Mes_anterior,
    (vpm.Total_vendido - dpm.Mes_anterior) AS Diferenca_pro_mes_anterior,
    CASE
		WHEN dpm.Mes_anterior IS NULL OR dpm.Mes_anterior = 0 THEN NULL 
...
FROM vendas_por_mes AS vpm
INNER JOIN diferenca_por_mes AS dpm ON vpm.Mes_ano = dpm.Mes_ano
ORDER BY Mes_ano;
